...
$steps = [
    [is_file(Maryam\BrandAssets::dir() . '/logo-white.png') && count(Maryam\BrandAssets::refs()) >= 3, "Dizayner: logoni va 3+ dizayn namunasini yuklang — tayyor rasmlar sizning uslubingizda chiqadi", 'brend'],
    [$brandFilled && $stats['knowledge'] + count($brand['facts'] ?? []) >= 3, 'Kompaniya → Faktlar: kompaniya faktlarini kiriting (tajriba, ofislar, afzalliklar)', 'bilimlar'],
    [count(Marketing::products()) > 0, "Kompaniya → Turlar: sotuvdagi turlarni narx va sanasi bilan kiriting — agentlar [NARX] qoldirmaydi", 'katalog'],
    [$stats['examples'] >= 5, "Namunalar: eng yaxshi 5-20 ta postingizni qo'shing (hozir {$stats['examples']} ta)", 'namunalar'],
    [$stats['templates'] >= 3, "Shablonlar: sahifangiz uchun doimiy post tuzilmalari (hozir {$stats['templates']} ta faol)", 'shablonlar'],
    [$stats['rated'] >= 10, "Copywriter'da kamida 10 ta natijani baholang va izoh yozing (hozir {$stats['rated']} ta)", 'studio'],
...
<div class="grid2">
  <div class="card"><h3>1. Siz o'rgatasiz</h3><p class="small muted">Shablon, qoida va namunalarni shu yerda, turlar va faktlarni Kompaniya bo'limida kiritasiz.</p></div>
  <div class="card"><h3>2. Agentlar yozadi</h3><p class="small muted">Copywriter yoki Kontent-strateg: strateg → copywriter → muharrir. Muharrir qoidalar va shablonga mosligini tekshiradi.</p></div>
...
<?php if ($stats['unrated']): ?>
  <p class="muted small"><?= $stats['unrated'] ?> ta natija hali baholanmagan — <a href="<?= e(url(['p' => 'studio'])) ?>">Copywriter</a>.</p>
<?php endif; ?>
